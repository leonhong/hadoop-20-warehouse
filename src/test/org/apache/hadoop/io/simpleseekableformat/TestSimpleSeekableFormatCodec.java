...
public class TestSimpleSeekableFormatCodec extends TestCase {
  ...
  void testTextInputFormat(final Class<? extends CompressionCodec> codecClass,
    ...
    // Write an SSF file to the FileSystem
    {
      ...
      if (truncateSize >= 0) {
        fileOut = new TestUtils.TruncatedOutputStream(fileOut, truncateSize);
      }
    ...
    for (int r = 0; r < numRecords; r++) {
      boolean gotData = recordReader.next(key, value);
      if (truncateSize < 0) {
    ...
    // Verify EOF
    Assert.assertFalse(recordReader.next(key, value));
    recordReader.close();

    // Print stats
    System.out.println("File length=" + fileStatus.getLen() + " records=" + numRecords
